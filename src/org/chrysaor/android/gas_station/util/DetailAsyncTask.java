...
public class DetailAsyncTask extends AbstractLoadingTask<String, Void, AsyncTaskResult<GSInfo>> {
    ...
    public DetailAsyncTask(Context context, View viewLoading, View viewMain, DetailTaskCallback callback) {
        super(context, viewLoading, viewMain);
    ...
    protected void onPostExecute(AsyncTaskResult<GSInfo> result) {
        ...
        // ブランド
        ImageView imgBrand = (ImageView) viewMain.findViewById(R.id.brand_image);
        StandsHelper helper = StandsHelper.getInstance();
        ...
        // 店名
        TextView textShopName = (TextView) viewMain.findViewById(R.id.shop_text);
        textShopName.setText(info.ShopName);

        // 住所
        TextView textAddress = (TextView) viewMain.findViewById(R.id.address_text);
        textAddress.setText(info.Address);

        // 距離
        TextView textDistance = (TextView) viewMain.findViewById(R.id.distance_text);
        ...
        new Thread(new Runnable() {
            public void run() {
                //画像
//                Bitmap imgBitmap;
                final ImageView imgView = (ImageView) viewMain.findViewById(R.id.shop_image);

                final ProgressBar progressBar = (ProgressBar) viewMain.findViewById(R.id.ProgressBar01);
...
//                imgBitmap = WebApi.getImageBitmapOnWeb(url, imgBitmap);
        ...
        txtRegularPrice = (TextView) viewMain.findViewById(R.id.txt_regular_price);
        txtRegularPrice.setTextSize(PRICE_FONT_SIZE);
        txtRegularPrice.setTypeface(tf);

        txtHighocPrice  = (TextView) viewMain.findViewById(R.id.txt_highoc_price);
        txtHighocPrice.setTextSize(PRICE_FONT_SIZE);
        txtHighocPrice.setTypeface(tf);

        txtDieselPrice  = (TextView) viewMain.findViewById(R.id.txt_diesel_price);
        ...
        txtRegularDate  = (TextView) viewMain.findViewById(R.id.txt_regular_date);
        ...
        txtMemRegularPrice = (TextView) viewMain.findViewById(R.id.txt_mem_regular_price);
        txtMemRegularPrice.setTextSize(PRICE_FONT_SIZE);
        txtMemRegularPrice.setTypeface(tf);

        txtMemHighocPrice  = (TextView) viewMain.findViewById(R.id.txt_mem_highoc_price);
        txtMemHighocPrice.setTextSize(PRICE_FONT_SIZE);
        txtMemHighocPrice.setTypeface(tf);

        txtMemDieselPrice  = (TextView) viewMain.findViewById(R.id.txt_mem_diesel_price);
        txtMemDieselPrice.setTextSize(PRICE_FONT_SIZE);
        txtMemDieselPrice.setTypeface(tf);

        txtMemLampPrice    = (TextView) viewMain.findViewById(R.id.txt_mem_lamp_price);
        txtMemLampPrice.setTextSize(PRICE_FONT_SIZE);
        txtMemLampPrice.setTypeface(tf);
        
        txtMemRegularDate  = (TextView) viewMain.findViewById(R.id.txt_mem_regular_date);
        txtMemHighocDate   = (TextView) viewMain.findViewById(R.id.txt_mem_highoc_date);
        txtMemDieselDate   = (TextView) viewMain.findViewById(R.id.txt_mem_diesel_date);
        txtMemLampDate     = (TextView) viewMain.findViewById(R.id.txt_mem_lamp_date);
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            for (Price price : prices) {
                                ...
                                // レギュラー（現金価格）
                                if (mode == GoGoGsApi.MODE_REGULAR && GoGoGsApi.isMember(member) == false) {
                                    Utils.logging("price:" + price.getPrice().toString());
                                    Utils.logging("date:" + price.getDate().toString());
                                    txtRegularPrice.setText(price.getPrice().toString());
                                    txtRegularDate.setText(price.getDate());
                                }
                                
                                // ハイオク（現金価格）
                                if (mode == GoGoGsApi.MODE_HIGHOC && GoGoGsApi.isMember(member) == false) {

                                    txtHighocPrice.setText(price.getPrice().toString());
                                    txtHighocDate.setText(price.getDate());
                                }

                                // 軽油（現金価格）
                                if (mode == GoGoGsApi.MODE_DIESEL && GoGoGsApi.isMember(member) == false) {
                                    txtDieselPrice.setText(price.getPrice().toString());
                                    txtDieselDate.setText(price.getDate());
                                }

                                // 灯油（現金価格）
                                if (mode == GoGoGsApi.MODE_LAMP && GoGoGsApi.isMember(member) == false) {

                                    txtLampPrice.setText(price.getPrice().toString());
                                    txtLampDate.setText(price.getDate());
                                }
                                
                                // レギュラー（会員価格）
                                if (mode == GoGoGsApi.MODE_REGULAR && GoGoGsApi.isMember(member) == true) {
                                    txtMemRegularPrice.setText(price.getPrice().toString());
                                    txtMemRegularDate.setText(price.getDate());

                                }

                                // ハイオク（会員価格）
                                if (mode == GoGoGsApi.MODE_HIGHOC && GoGoGsApi.isMember(member) == true) {
                                    txtMemHighocPrice.setText(price.getPrice().toString());
                                    txtMemHighocDate.setText(price.getDate());
                                }

                                // 軽油（会員価格）
                                if (mode == GoGoGsApi.MODE_DIESEL && GoGoGsApi.isMember(member) == true) {
                                    txtMemDieselPrice.setText(price.getPrice().toString());
                                    txtMemDieselDate.setText(price.getDate());
                                }

                                // 灯油（会員価格）
                                if (mode == GoGoGsApi.MODE_LAMP && GoGoGsApi.isMember(member) == true) {
                                    txtMemLampPrice.setText(price.getPrice().toString());
